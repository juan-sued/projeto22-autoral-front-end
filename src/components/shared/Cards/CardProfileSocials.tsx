...
import imageMock from '@/assets/ale-foto.jfif';
import { useInView } from 'react-intersection-observer';
export default function CardProfileSocials() {
  const { ref, inView } = useInView({
    delay: 200,
    threshold: 0
  });
  return (
    <CardProfileSocialsStyle imageMock={imageMock} inView={inView} ref={ref}>
      <div className="card">
        <div className="card-info">
          <div className="card-avatar"></div>
          <div className="card-title">Alessandra Meireles</div>
          <div className="card-subtitle">20 criações &amp; 3510 likes</div>
        </div>
    ...
    </CardProfileSocialsStyle>
  );
}

interface CardProfileSocialsStyleProps {
  imageMock: string;
  inView: boolean;
}

const CardProfileSocialsStyle = styled.div<CardProfileSocialsStyleProps>`
  opacity: 0;
  ${props =>
    props.inView
      ? `animation: fadeTranslate 0.8s ease-in-out;animation-fill-mode: forwards;`
      : ''}
  ...
  .card-avatar {
    ...
    margin-bottom: 1rem;
    background-image: url(${props => props.imageMock});
  ...
  .card-title {
    ...
    :hover {
      cursor: pointer;
      text-decoration: underline #53ef7d 1px;
    }
  }
